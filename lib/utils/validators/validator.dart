...
class Validator {
  ...
  static Map<String, String>? splitPhoneNumber(String fullNumber) {
    final RegExp regExp = RegExp(r'^\+(\d{1,4})(\d+)$');
  ...
  static RegExp phoneNumberWithCCRegExp = RegExp(r'^\+(\d{1,3})(\d+)$');

  // static const removeLeadingWhiteSpace = r'^\s+';
  ...
  static emojiRestrict() {
    return FilteringTextInputFormatter.deny(RegExp(regexToRemoveEmoji));
  }
  ...
  static bool isUsNumber(String number) {
    const String pattern =
        r'^(?:\([0-9]{3}\)\s?|[0-9]{3}[.\-]?)?[0-9]{3}[.\-]?[0-9]{4}$';
    final regExp = RegExp(pattern);
    return regExp.hasMatch(number.trim());
  }
