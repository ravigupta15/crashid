...
class OtherAccidentNotifier extends AsyncNotifier<OtherAccidentState> {
      ...
      if (response?.statusCode == 201 || response?.statusCode == 200) {
        state = AsyncData(
          state.value!.copyWith(
            pricingResponseModel: PricingResponseModel.fromJson(response?.data),
          ),
        );
      }
    } catch (_) {
  ...
  Future complete(OtherAccidentSendModel? sendModel, {String? route}) async {
    ...
    try {
      ...
      if (response?.statusCode == 201 || response?.statusCode == 200) {
        ...
        _openPaymentWebviewScreen(
          caseId: sendModel?.caseId,
  ...
  Future paymentcapture({
    ...
    try {
      final repo = ref.read(accidentRepositoryProvider);
      final response = await repo.paymentCapture(
        caseId: caseId,
      ...
      if (response?.statusCode == 201 || response?.statusCode == 200) {
        ...
        if (route == "case_details") {
          ...
          // Navigator.pop(context);
          // Navigator.pop(context);
          AppNavigationScreen.open(context, initialIndex: 3);
        }
      }
    } catch (_) {
    } finally {
  ...
  void _openPaymentWebviewScreen({
    String? caseId,
    String? paymentUrl,
    String? route,
  }) {
